    ...
    $.extend($.summernote.plugins, {
        imageAttributes: function (context) {
            ...
            this.initialize = function () {
                ...
                var $container = options.imageAttributes.dialogsInBody
                    ? $(document.body)
                    : $editor;
                    ...
                    if (isUpdateCaption && captionText) {
                        ...
                        if ($parentAnchorLink.is("a")) {
                            $newFigure = $parentAnchorLink
                                .wrap(
                                    `<figure class="${options.imageAttributes.figureClass}"></figure>`
                                )
                                .parent();
                            $newFigure.append(
                                `<figcaption class="${options.imageAttributes.figcaptionClass}"> ${captionText}</figcaption>`
                            );
                        } else {
                            $newFigure = $img
                                .wrap(
            ...
            this.showLinkDialog = function (imgInfo) {
                return $.Deferred(function (deferred) {
                    ...
                    var $imageCaption = self.$dialog.find(
                    ...
                    var $imageWidthInput =
                        self.$dialog.find(".note-image-width");
                    var $imageHeightInput =
                        self.$dialog.find(".note-image-height");
                    var $lockButton = self.$dialog.find(".lock-button");
                    var $resetSizeButton = self.$dialog.find(
                        ".reset-size-buttton"
                    );
                    ...
                    var isLocked =
                        typeof options.imageAttributes.manageAspectRatio ===
                        "undefined"
                            ? true
                            : options.imageAttributes.manageAspectRatio;
                    if (isLocked) {
